...
fn main() {
    ...
    let mut intermedOne = Arc::new(Mutex::new(0));
    let mut intermedOneClone = intermedOne.clone();
    let mut intermedTwo = Arc::new(Mutex::new(0));
    let mut intermedTwoClone = intermedTwo.clone();
    ...
    let t1 = thread::spawn(move ||*intermedOne.lock().unwrap() = map_data(&i1));
    let t2 = thread::spawn(move ||*intermedTwo.lock().unwrap() = map_data(&i2));

    let r1 = t1.join().unwrap();
    let r2 = t2.join().unwrap();

    intermediate_sums.push(*intermedOneClone.lock().unwrap());
    intermediate_sums.push(*intermedTwoClone.lock().unwrap());
    ...
    for i in 0..num_partitions{
      ...
      threads.push(thread::spawn(move || ret.lock().unwrap()[i] = map_data(&data)));
    }
    ...
    for i in 0..num_partitions{
      intermediate_sums.push(result.lock().unwrap()[i]);
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let partition_mod = v.len() % num_partitions;
    let mut xs: Vec<Vec<usize>> = Vec::new();
    ...
    for i in 0..num_partitions{
      ...
      if (partition_mod != 0) && (i == (num_partitions-1))
      {
        for j in 0..(partition_size-(num_partitions-partition_mod)){
          x1.push(v[j + (i * partition_size)]);
        } 
      }
      else
      {
